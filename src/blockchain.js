...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            if(self.chain.length > 0) {block.previousBlockHash = self.chain[self.chain.length -1].hash;}
            block.time = new Date().getTime().toString().slice(0,-3);
            block.height = self.chain.length;
            block.hash = SHA256(JSON.stringify(block)).toString();
            
            self.chain.push(block);
            if(self.chain[self.chain.length -1].hash == block.hash){
                self.height +=1;
                resolve(block);
            }
            else{
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let new_date = new Date().getTime().toString().slice(0,-3);
            let new_address = address +":"+ new_date +":"+ 'starRegistry';
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime - message_time < 300){reject();}
            let verified = bitcoinMessage.verify(message, address, signature);
             if(verified){
                ...
                let new_block = new BlockClass.Block(block_data);
                let block_return = self._addBlock(new_block);
                console.log(self.chain);
                resolve(block_return); let block_data = {
                    "owner": address,
                    "star":star
                };
                let new_block = new BlockClass.Block(block_data);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            for (let i=0; i<self.chain.length;i++){
                if(JSON.parse(hex2ascii(self.chain[i].body))['owner'] === address){
